...
# ──────────────────────────────────────────────────────────────
# 1. Universal-IO matrix: TILE input × output combos
# ──────────────────────────────────────────────────────────────
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Decomposable (N=0): composite chains
        pytest.param(
            (2, 4, 32, 64),
            (0, 2, 3, 1),
            ttnn.TILE_LAYOUT,
...
def test_permute_universal_io(shape, dims, input_layout, input_factory, output_factory, dtype, device):
    if dtype == ttnn.bfloat8_b and input_layout == ttnn.ROW_MAJOR_LAYOUT:
        pytest.skip("bfloat8_b requires TILE_LAYOUT")
    in_mc = input_factory(device)
    if dtype == ttnn.float32 and input_layout == ttnn.ROW_MAJOR_LAYOUT and in_mc.is_sharded():
        if in_mc.memory_layout == ttnn.TensorMemoryLayout.HEIGHT_SHARDED:
            pytest.xfail("RM HEIGHT_SHARDED WH transpose corrupts f32 elements (#41662)")
    if dtype == ttnn.float32 and dims == (3, 2, 1, 0) and in_mc.is_sharded():
        pytest.skip("Full-reverse permutation on f32 creates per-core shards exceeding L1")
    run_permute_test(
        shape,
        dims,
        device,
...
def test_permute_cross_shard_height_to_width(device):
    """Height-sharded input → width-sharded output with explicit shard_spec."""
    shape = (1, 1, 64, 128)
    run_permute_test(
        shape,
        (0, 1, 3, 2),
...
def test_permute_cross_shard_block_to_height(device):
    """Block-sharded input → height-sharded output with explicit shard_spec."""
    shape = (1, 1, 64, 64)
    run_permute_test(
        shape,
        (0, 1, 3, 2),
...
# ──────────────────────────────────────────────────────────────
# 7. ROW_MAJOR sharded input combos (composite fallback)
# ──────────────────────────────────────────────────────────────
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # RM non-decomposable + width/block sharded (exercises RM workaround on prim::permute path)
        pytest.param(
            (2, 4, 32, 64),
            (2, 0, 1, 3),
        ...
        # RM height-sharded non-decomposable (direct sharded read via TensorAccessor, no RM workaround)
        pytest.param(
            (2, 4, 32, 64),
            (2, 0, 1, 3),
            lambda d: _height_shard_config((2, 4, 32, 64), d, layout=ttnn.ROW_MAJOR_LAYOUT),
...
# RM interleaved → sharded output without shard_spec.
@pytest.mark.parametrize(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            (0, 2, 1, 3, 4),
            ttnn.ROW_MAJOR_LAYOUT,
            lambda d: _width_shard_config((2, 2, 2, 32, 64), d, num_cores=4, layout=ttnn.ROW_MAJOR_LAYOUT),
            id="5d_rm_width_sharded",
        ),
        pytest.param(
            (2, 2, 2, 32, 64),
            (0, 2, 1, 4, 3),
            ttnn.TILE_LAYOUT,
            lambda d: _block_shard_config((2, 2, 2, 32, 64), d),
            id="5d_tile_block_sharded",
        ),
        pytest.param(
            (2, 2, 2, 32, 64),
            (0, 2, 1, 3, 4),
