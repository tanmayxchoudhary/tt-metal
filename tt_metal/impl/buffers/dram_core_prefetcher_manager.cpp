...
std::vector<uint8_t> DramCorePrefetcherManager::serialize_request_page(
    ...
    header->prefetch.num_tensors = static_cast<uint16_t>(num_tensors);
    header->prefetch.num_layers = num_layers;
...
void DramCorePrefetcherManager::enqueue_cq_signal_and_wait(
    ...
    // The destination is the full device address (the kernel's local slot base plus
    // the programmable-DRAM-core L1 NOC offset), so the dispatcher must NOT apply a
    // bank offset.
    const uint64_t dram_l1_noc_offset = MetalContext::instance(mesh_device_->impl().get_context_id())
                                            .hal()
                                            .get_l1_noc_offset(HalProgrammableCoreType::DRAM);
...
void WaitForCqOnDramCorePrefetcher(
    ...
    uint8_t cq_id,
    const std::optional<distributed::MeshCoordinateRangeSet>& device_subset) {
    TT_FATAL(mesh_device != nullptr, "WaitForCqOnDramCorePrefetcher requires a non-null MeshDevice");
    auto& manager = mesh_device->impl().dram_core_prefetcher(mesh_device);
