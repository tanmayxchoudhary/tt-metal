...
def _seed_manager_debug_summary(seed_manager, slots=None):
    ...
    for slot in slots:
        ...
        counter = seed_manager.seed_counters[slot]
        if seed is not None or counter:
            summary.append((slot, seed, counter))
...
class Generator(WarmupForwardMixin):
    ...
    def prefill_forward_text(
        ...
        _log_sampling_debug(
            ...
            explicit_seeded_prefill=explicit_seeded_prefill,
            requires_slot_stable_prefill=requires_slot_stable_prefill,
        ...
        if do_device_sampling:
            ...
            if explicit_seeded_prefill:
                ...
                for request_idx, slot in enumerate(empty_slots):
                    ...
                    _log_sampling_debug(
                        self._sampling_debug_enabled,
                        "Galaxy seeded prefill request",
                        request_idx=request_idx,
                        slot=slot,
                        single_params=_sampling_params_debug_summary(single_params),
                        seed_state_before=_seed_manager_debug_summary(seed_manager, [slot]),
                    )
                    ...
                    _log_sampling_debug(
                        self._sampling_debug_enabled,
                        "Galaxy seeded prefill sampled",
                        request_idx=request_idx,
                        slot=slot,
                        sampled_token=int(sampled_token),
                        seed_state_after=_seed_manager_debug_summary(seed_manager, [slot]),
                    )
    ...
    def decode_forward(
        ...
        if self._decode_inputs_need_reset:
            reset_inputs = True
            self._decode_inputs_need_reset = False
            reset_reasons.append("after_prefill")
        if sampling_params is not None:
            ...
            if has_greedy and has_random:
                reset_inputs = True
                reset_reasons.append("mixed_greedy_random")
        ...
        _log_sampling_debug(
            ...
            active_seed_slots=_compact_debug_list(active_seed_slots),
            start_pos=_compact_debug_list(start_pos),
            tokens=_tensor_debug_summary(tokens),
            page_table=_tensor_debug_summary(page_table),
            sampling_params=_sampling_params_debug_summary(sampling_params),
        ...
        _log_sampling_debug(
            self._sampling_debug_enabled,
            "Galaxy decode seeds advanced",
            active_seed_slots=_compact_debug_list(active_seed_slots),
            seed_state=_seed_manager_debug_summary(seed_manager, active_seed_slots),
        )
