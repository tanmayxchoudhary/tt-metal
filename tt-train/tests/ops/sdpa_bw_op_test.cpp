...
void run_sdpa_backward_test(const SDPABackwardTestConfig& config) {
    ...
    // Forward pass checks
    const bool fw_attn_output_matches = xt::allclose(kernel_attn_clean, composite_attn_output_cpu, rtol, atol);
    // Compare kernel intermediates (FP32 logsumexp) vs float reference (value at pos 0 only)
    const bool fw_intermediates_matches = xt::allclose(kernel_interm_clean, float_intermediates, rtol, atol);
