...
void fill_tile(const Noc& noc, uint32_t cb_id, uint32_t tile_id, uint32_t val) {
    if (val == 0) {
        ...
        UnicastEndpoint zeros_src;

        // Fill tile with zeros
        for (uint32_t i = 0; i < num_zeros_reads; ++i) {
            noc.async_read(
                zeros_src,
                CoreLocalMem<uint32_t>(write_addr),
                MEM_ZEROS_SIZE,
                {.noc_x = my_noc_x, .noc_y = my_noc_y, .addr = MEM_ZEROS_BASE},
                {});
...
void fill_tile_partial(
    const Noc& noc, uint32_t cb_id, uint32_t tile_id, uint32_t cur_pos_in_tile, uint32_t partial_val) {
    /*
    We want to fill cur_pos_in_tile + 1 to the end
    */
    constexpr int num_faces = (tile_bytes == 1024) ? 2 : 4;

    fill_tile<tile_bytes>(noc, cb_id, tile_id, 0);
...
void fill_tile_partial_sliding_window(
    const Noc& noc, uint32_t cb_id, uint32_t tile_id, uint32_t window_start_pos_in_tile, uint32_t partial_val) {
    /*
...
void generate_mask(const Noc& noc, uint32_t k_num_chunks, uint32_t Sk_chunk_t, uint32_t cur_pos) {
    /*
    ...
    */
    ...
    for (uint32_t i = 0; i < Sk_chunk_t; ++i) {
        if (i < cur_pos_in_chunk_t) {
            // fill with zero
            if (i == 0) {
                fill_tile<tile_bytes>(noc, cb_mask_in, i, 0);
            } else {
            ...
            fill_tile_partial<tile_bytes>(noc, cb_mask_in, i, cur_pos_in_tile, NEG_INF);
        } else {
            // fill with -inf
            if (i == cur_pos_in_chunk_t + 1) {
                fill_tile<tile_bytes>(noc, cb_mask_in, i, NEG_INF);
            } else {
                copy_tile<tile_bytes>(
                    noc,
                    q_write_ptr_base,
...
template <uint32_t cb_mask_in, uint32_t PNHt>
void generate_sliding_window_mask(const Noc& noc, uint32_t k_num_chunks, uint32_t Sk_chunk_t, uint32_t window_start) {
    /*
    ...
    */
    ...
    for (uint32_t i = 0; i < Sk_chunk_t; ++i) {
        if (i < window_start_in_chunk_t) {
            ...
            fill_tile_partial_sliding_window<tile_bytes>(noc, cb_mask_in, i, window_start_in_tile, NEG_INF);
        } else {
...
void generate_block_padding_mask(const Noc& noc, uint32_t Sk_chunk_t, uint32_t block_size) {
    ...
    for (uint32_t i = 0; i < Sk_chunk_t; ++i) {
        ...
        fill_tile_partial<tile_bytes>(noc, cb_block_pad_mask, i, block_size - 1, NEG_INF);
...
uint32_t write_tiles_to_memory(
    const Noc& noc, uint32_t& out_tile_id, const WriterType& out_writer, uint32_t& barrier_count) {
...
uint32_t read_k(
    ...
    if constexpr (use_mcast) {
        if (mcast_params.do_mcast) {
            ...
            mcast_sem.set(VALID);
            mcast_sem.template set_multicast<Noc::McastMode::EXCLUDE_SRC>(
                noc,
    ...
    for (uint32_t k_chunk = k_chunk_start; k_chunk < k_chunk_end; ++k_chunk) {
        ...
        if constexpr (use_attention_mask) {
            mask_start_tile_id = read_mask_chunk<cb_mask_in, mask_tile_bytes, barrier_threshold, PNHt>(
                noc, PSt, Sk_chunk_t, mask_chunk_tiles, mask_start_tile_id, mask_reader);
        }
