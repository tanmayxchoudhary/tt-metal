...
void dprint_cb_tile(const Noc& noc, uint32_t cb_id, uint32_t tile_id) {
        ...
        for (uint32_t nq = local_nh_start; nq < local_nh_end; ++nq) {
            ...
            for (uint32_t q_iter = 0; q_iter < q_chunks_per_core; ++q_iter) {
                ...
                read_chunk_with_padding<q_tile_bytes>(
                    noc, q_reader, cb_q_in, q_tile_id, q_row_tile_count, DHt, Sq_chunk_t, DHt, barrier_threshold);
                ...
                for (uint32_t k_chunk = 0; (k_chunk * Sk_chunk_t) < q_high_idx_in_tiles; ++k_chunk) {
                    ...
                    // Read K chunk
                    auto k_chunk_ntiles = read_chunk_with_padding<k_tile_bytes, decltype(k_reader), false>(
                        noc,
                        k_reader,
                        cb_k_in,
                        k_start_tile_id,
