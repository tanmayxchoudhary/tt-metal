 ...
 */
template <uint32_t num_cols, bool is_causal_sdpa, bool kv_pad_rotation_enabled = false>
    ...
    for (uint32_t row = 0; row < sbh; row++) {
        ...
        if constexpr (is_causal_sdpa) {
            if (apply_causal || kv_pad_rotation_enabled) {
                ...
                const uint32_t q_pos_u32 = q_global_tile_for_local_mask<kv_pad_rotation_enabled>(
                    q_tile,
                    q_start_tile,
                ...
                if (straddle_col == 0) {
                    ...
                    if (diag_col < 0) {
                        ...
                        for (uint32_t col = static_cast<uint32_t>(diag_col) + 1; col < mask_cols; col++) {
                            l1_acc_single_tile(mask_cb, neginf_idx, out_cb, row_offset + col);
                        }
                    }
                } else {
...
static void sdpa_inner_loop_step(
    ...
    const uint32_t kv_pad_q_old_start_nt = 0,
    const uint32_t kv_pad_q_old_count_nt = 0,
    const uint32_t kv_pad_q_new_start_nt = 0,
    const uint32_t kv_pad_q_valid_nt = 0) {
    ...
    for (uint32_t q_subblock = 0; q_subblock < q_num_subblocks; q_subblock++) {
        ...
        if constexpr (ring_mode || is_causal_sdpa || use_padded_mask) {
            ...
            if (should_apply_lightweight_mask) {
                ...
                PACK((llk_pack_reconfig_l1_acc(1)));
                apply_lightweight_mask_streaming<KT_stride, is_causal_sdpa, kv_pad_rotation_enabled>(
                    cb_mask_in,
                    cb_qkt_im,
                    q_subblock,
    ...
    for (uint32_t q = global_q_start; q < global_q_end; q++) {
        ...
        for (uint32_t k_chunk = 0; k_chunk < num_kv_chunks; ++k_chunk) {
            ...
            sdpa_inner_loop_step<
                ...
                step_straddle_col,
                step_straddle_jump,
                chunked.kv_pad_q_old_start_nt,
                chunked.kv_pad_q_old_count_nt,
                chunked.kv_pad_q_new_start_nt,
                chunked.kv_pad_q_valid_nt);
