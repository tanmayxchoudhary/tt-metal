...
void kernel_main() {
    ...
    // For the unicast path, we still need the raw L1 addresses for set_remote (legacy free function).
    const uint32_t valid_semaphore_l1_addr = get_semaphore(valid_semaphore_id);
    const uint32_t receiver_semaphore_l1_addr = get_semaphore(receiver_semaphore_id);

    // Pre-set VALID semaphore to VALID — receiver path waits on remote setting it back to VALID.
            ...
            for (uint32_t k_chunk = 0; k_chunk < num_kv_chunks; ++k_chunk) {
                ...
                if (should_forward) {
                    ...
                    if constexpr (mcast_enabled) {
                        ...
                        // Companion semaphore mcast: write local valid_sem value to remote
                        // receiver_sem (different L1 offset). See reader_interleaved.cpp for rationale.
                        ...
                        noc_semaphore_set_multicast(
                            valid_semaphore_l1_addr, mcast_sem_noc_addr, mcast_num_dests, false, noc.get_noc_id());
                    } else {
                        noc.async_write(
                            CoreLocalMem<uint32_t>(cb_k_start_address),
                            UnicastEndpoint{},
                    ...
                    if constexpr (!mcast_enabled) {
                        const uint64_t remote_receiver_noc_addr = ::get_noc_addr(
                            next_physical_x, next_physical_y, receiver_semaphore_l1_addr, noc.get_noc_id());
                        noc_semaphore_set_remote(valid_semaphore_l1_addr, remote_receiver_noc_addr, noc.get_noc_id());
                    }
                }
                ...
                if (k_chunk == 0 && need_q_read) {
                    if constexpr (use_q_subblock_push) {
                        ...
                        for (uint32_t q_sub = 0; q_sub < q_num_subblocks; ++q_sub) {
                            ...
                            read_block(
                                noc, q_gen, q_sub_slice, q_end_seq_tile, cb_q_in, q_tile_bytes, false /*transpose*/
                            );
                        }
                    } else {
                        read_block(
