...
FORCE_INLINE void read_chunk_for_forwarding(
    ...
    for (uint32_t row = 0; row < dst_rows; ++row) {
        for (uint32_t col = 0; col < dst_cols; ++col) {
            ...
            fill_zeros_async(noc, dst_addr + tile_idx * tile_bytes, tile_bytes);
        }
    }
...
void kernel_main() {
    ...
    if constexpr (!is_causal) {
        ...
        if (is_chain_participant) {
            ...
            receiver_semaphore_l1_addr = get_semaphore(receiver_semaphore_id);
            ...
            */
            ...
            if constexpr (!use_q_subblock_push) {
                read_chunk_with_padding<q_tile_bytes>(
                    noc, q_reader, cb_q_in, q_read_tile_id, q_row_tile_count, DHt, Sq_chunk_t, DHt, barrier_threshold);
            }
            ...
            for (uint32_t k_chunk = 0; (k_chunk * Sk_chunk_t) < q_high_idx; ++k_chunk) {
                ...
                if (should_forward) {
                    if constexpr (!mcast_enabled) {
                        ...
                        noc_semaphore_set_remote(valid_semaphore_addr, remote_receiver_noc_addr, noc.get_noc_id());
                    }
                }
