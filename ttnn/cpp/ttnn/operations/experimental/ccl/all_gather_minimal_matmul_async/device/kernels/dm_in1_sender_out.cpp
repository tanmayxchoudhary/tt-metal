...
void kernel_main() {
...
#ifdef FSDP_FUSED
    constexpr uint32_t fsdp_ring_size = get_compile_time_arg_val(20);
    constexpr uint32_t fsdp_ring_index = get_compile_time_arg_val(21);
    constexpr uint32_t fsdp_num_targets_forward = get_compile_time_arg_val(22);
    constexpr uint32_t fsdp_num_targets_backward = get_compile_time_arg_val(23);
    constexpr Topology fsdp_topology = static_cast<Topology>(get_compile_time_arg_val(24));
    ...
    constexpr uint32_t fsdp_mux_arg_count = 30;
...
#else
    constexpr uint32_t ct_arg_count = 20;
#endif
    ...
    for (uint32_t m_block_iter = 0; m_block_iter < M_blocks_per_core; m_block_iter++) {
        ...
        for (uint32_t n_block_iter = 0; n_block_iter < N_blocks_per_core; n_block_iter++) {
            ...
            for (uint32_t k_block_iter = 0; k_block_iter < K_num_blocks; k_block_iter++) {
                ...
                uint32_t in1_send_l1_addr = in1_start_address;  // remembered for fabric send (NOC chain block below
                                                                // increments in1_start_address)
