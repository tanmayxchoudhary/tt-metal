...
4. [Migration Patterns](#migration-patterns)
   ...
   - [State Options Structs](#state-options-structs)
...
```cpp
struct NocOptVals {
    uint8_t vc   = NOC_UNICAST_WRITE_VC;  // used when NocOptions::CUSTOM_VC
    uint8_t trid = 0;                     // used when NocOptions::TXN_ID
};
```
...
| Custom vc + trid (reads only) | `NocOptions::TXN_ID \| NocOptions::CUSTOM_VC` | `NocOptVals{.vc = v, .trid = t}` |
